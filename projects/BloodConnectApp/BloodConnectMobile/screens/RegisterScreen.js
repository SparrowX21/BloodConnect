...
const RegisterScreen = ({ navigation, route }) => {
  ...
  const handleRegister = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`http://${SERVER_IP}:3001/api/register`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name, email, bloodType, pincode, phone }),
      });
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput
        style={styles.input}
        placeholder="Pin Code (75025 etc ) "
        value={pincode}
...
const styles = StyleSheet.create({
  ...
  picker: { height: 50 },
  button: { backgroundColor: '#e74c3c', padding: 15, borderRadius: 10, alignItems: 'center', marginTop: 20 },
